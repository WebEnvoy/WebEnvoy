...
- 本轮 [#519](https://github.com/WebEnvoy/WebEnvoy/issues/519) B 已形成只接受 owner 明确提供、重新核验 provenance 的供应方原版 Camoufox 路径：Camoufox `0.5.6`、browser `152.0.4-beta.30`、Playwright `1.60.0`，由公开 API Driver 消费完整 launch/context options 并精确复用。该路径当前按 `limited` 处理：popup 首请求若在派发前无法建立可信 Page 归属，必须在任何 `fetch`/`continue`/外部请求前以 `page_relation_unavailable` 局部拒绝，不猜测、不重放；普通任务页和同 Instance 观察/接管语义仍有效。正式 installed、人工交还、环境连续性和真实 Agent 消费仍须通过 #519 完成门，当前不能写成 `live_verified`。旧 Camoufox 私有 launch binding、patched/native artifact 继续 `unsupported`／已退役，不启动、不 fallback；#499、#504、#510 的旧记录仅作历史/恢复校验事实。
