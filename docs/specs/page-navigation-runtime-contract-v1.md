...
The deterministic Harbor evidence is kept in `services/harbor/packages/runtime-api/src/page-navigation.test.ts` and the real listener coverage in `runtime-diagnostics.test.ts`; #510 relation, artifact and CSS integrity evidence is historical and linked from [Camoufox Native Provider Contract V1](camoufox-native-provider-contract-v1.md). The #519 upstream Driver's pre-dispatch unknown-relation and popup-origin checks are separately covered by `services/harbor/packages/runtime-api/src/camoufox-upstream-driver.test.ts`; those tests are fixture/code evidence and remain distinct from #519/PR #522's installed/live/plugin evidence. That installed evidence applies only to its stated original-page and handoff scope and does not claim complete native focus, every popup/window route, every platform, or complete Runtime support. The old private Camoufox launch binding remains retired; the current official path retains its documented limited popup boundary.
