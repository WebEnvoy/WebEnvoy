...
因此第8节 1–7 项的运行时门由标注为 f0 的确定性安装证据覆盖，当前 47d exact 安装另外复验了安装身份、origin 引导和真实 Agent 文件闭环；两者均未改变固定 Provider、权限门或验收标准。保留的 harness 顺序/CLI 路径和 Page event-loop 诊断为验收过程证据，不是产品失败或范围变更。
