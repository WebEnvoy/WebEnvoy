...
5. Replay passes a deep copy of the complete stored `config` to `launch_options(config=...)`. Because pinned `merge_into()`/`set_into()` preserve existing keys, the stored fingerprint, fonts, voices, WebGL values and seeds are retained. The replay result is re-hashed; a changed identity hash fails closed.
...
| Canvas seed | Provider randomizes once per first config and stores `canvas:seed` | Full config replay | `canvas_hash` only; fixed local drawing is continuity-checked when non-null | Hash is not a raw seed or proof of every canvas path |
