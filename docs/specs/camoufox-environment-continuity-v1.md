...
| Timezone | WebEnvoy configured; dynamic and excluded from identity hash | In-memory `config["timezone"]` plus native `timezone_id` on the persistent context | `timezone`; IANA input validation does not replace actual browser readback | GeoIP-derived timezone is not enabled by this Driver |
