...
| [Network Runtime Contract V1](network-runtime-contract-v1.md) | [Work Item #498](https://github.com/WebEnvoy/WebEnvoy/issues/498)、[#594](https://github.com/WebEnvoy/WebEnvoy/issues/594) | 冻结 bounded Network metadata、Page binding、cursor、脱敏和生命周期语义；#594 的程序侧匿名公共读取为候选增量，合入前不生效。 |
...
| [Site SKILL Execution V1](site-skill-execution-v1.md) | [S2 #563](https://github.com/WebEnvoy/WebEnvoy/issues/563)、[资产 FR #475](https://github.com/WebEnvoy/WebEnvoy/issues/475)、[#594](https://github.com/WebEnvoy/WebEnvoy/issues/594) | 固定版本站点任务的准入、调用、输入输出、执行边界、目标新鲜度及失败恢复；#594 的无 Page 公共读取是候选增量，合入前不生效。Lode 唯一拥有包格式，规格不代替真实站点验收。 |
| [AccountSystem Local Definition V1](account-system-local-definition-v1.md) | [资产 FR #475](https://github.com/WebEnvoy/WebEnvoy/issues/475) | Accepted：Core 通过 owner API 导入固定 Lode 模板、维护本地不可变修订并供声明依赖的正式消费者使用；无身份推断、绑定或登录语义。 |
...
| 代号 | 真实 Issue | 计划归口 |
| --- | --- | --- |
| S4 | [#565](https://github.com/WebEnvoy/WebEnvoy/issues/565) | 完整主动 Network 能力；#594 的限定匿名公共读取候选不关闭本项。 |
...
- #594：`DO-NETWORK-CONTRACT = triggered`，公共 HTTP 正文与程序侧请求边界由 [Network 的 #594 候选补充](network-runtime-contract-v1.md#594-限定的程序侧匿名公共读取v13-候选) 承接，仅覆盖公共匿名只读，完整 S4 #565 仍开放。`DO-PLUGIN-EXPOSURE = triggered`，固定任务发现与提交映射见 [Plugin exposure](plugin-runtime-exposure-v1.md#594-公共只读-adapter-投影候选)；`DO-GRANT-WIRE = triggered`，无 Page 任务仍绑定现有 Profile/origin 与 Grant 交集，见 [Grant wire](grant-wire-contract-v1.md)。`DO-CONSOLE-CONTRACT`、`DO-PROVIDER-PRIVATE-SCHEMA`、`DO-APP-IA = not-triggered`：不新增 Console payload、Provider 私有配置或 Desktop App 入口。对应 Lode 包/导入合同由 Lode 自身修订；这些候选均不等于代码准入或实际站点通过。
