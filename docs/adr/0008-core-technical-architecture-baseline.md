...
Core Runtime 负责 admission、resource matching、action risk、run lifecycle 和执行控制。它只消费 Lode capability/package/schema/resource/post-check refs 与 Harbor runtime/profile/session/evidence facts，不直接拥有站点知识、浏览器现场、Profile、Viewer、provider driver 或 App UI state。
...
| Core Runtime | 做 admission、resource matching、action risk、run lifecycle、failure taxonomy 和执行控制。 | 不直接调用 Harbor private internals 或 Lode package body。 |
...
| True-write runtime | 写侧 Work Item | 必须先满足 approval、idempotency、post-check、unknown outcome 和 reconciliation。 |
...
| Core Runtime | admission、resource matching、action risk、run lifecycle、execution orchestration、failure taxonomy。 | Lode package/schema/resource/post-check refs，Harbor runtime facts/evidence refs，App approval/intent refs。 | Browser process、Profile storage、provider driver、site adapter implementation、business strategy。 |
...
| App Work / Library / Browser UI | App | `entrypoint_ref`、caller category、user intent summary、approval/recovery/cancel/reconcile intent refs。 | UI component state、draft editor content、local display cache、Settings secrets。 |
...
| `/Volumes/2T/dev/WebEnvoy/App/docs/contracts/README.md` | 吸收 | Core 只消费 App intent/approval/recovery refs；App 不生成 Run Record truth。 |
