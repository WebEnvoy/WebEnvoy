...
Lode action declaration matcher 或 Harbor operation/facts matcher 产出的唯一匹配证明。Core
用模块私有运行时品牌区分 matcher proof 与普通对象；evaluator 不解析调用方传入的裸
owner/category/scope/resource 声明。证明至少绑定：
...
构造器只从 owner contract 唯一匹配项派生类别、目标范围和资源要求，不能从 candidate
action 复制这些字段。缺少证明、证明冲突、重复 action/category、目标或资源不匹配、
无法分类，或出现 browser_fill/click/type/scroll 原子动作时一律停止，不能通过确认扩大
owner 声明边界。#292 只定义并验证该 proof 合同；真实 Lode/Harbor adapter 接入由 #293 完成。
...
### Evaluator 输入与后续入口一致性

#292 暴露的纯 evaluator 对 `unknown` 输入执行严格 parser；未知 caller、字段、枚举、非法
RFC3339 时间或低层浏览器原子动作稳定返回 `invalid_input` 并停止，不抛异常。当前合同只
接受最多毫秒精度的 RFC3339 时间，以便使用毫秒时钟做精确边界比较；更高精度输入拒绝，
不得先截断再比较。目标 origin 拒绝 URL userinfo，并规范化为 origin-only；credential-bearing
path/query 不进入决定输出。

本 ADR 要求 App、CLI、MCP、API、SDK、Agent 和非 task 系统操作最终复用同一 evaluator，
但 #292 不声称这些现有入口已经接线。各入口传入当前 owner facts、目标与策略来源，并在
#293 的 adapter 集成后获得一致行为，不建立入口特例。
...
最终公共合同由 `authorization-decision.v0` 与 `authorization-decision-ref.v0` Schema 承载；
task 的 Run Record/turn 和非 task Harbor operation record 只保存统一 opaque ref。
