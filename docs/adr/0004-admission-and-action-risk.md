...
- 动作风险、执行意图、审批证据和幂等追踪的最终 Schema。
- 四个最小风险类别是否足够支撑 App 策略和 MCP/CLI 展示。
- 审批有效期、撤销行为，以及与操作、目标、载荷和身份的绑定方式。
- 写任务锁粒度：会话、标签页、Profile、身份、能力还是目标对象。
- 每个动作风险类别的最小证据要求。
- 人工接管、验证码和登录恢复如何映射到准入或终态结果。
