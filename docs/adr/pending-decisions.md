...
> 2026-09-06：产品方向与实施顺序由 [ADR 0011](0011-v1-managed-browser-and-skill-delivery.md) 继任。下列历史条目只有在当前或下一批真实消费者仍受阻时才恢复为 blocker；对象／合同未“完善”本身不再构成阻塞。
...
- 阻塞什么：#341 后续 consumer 与 `cutover-ready`，尤其 Core #342 和 Harbor #352；最终跨仓 wire schema/API、version/hash pin 传输形式，以及 Harbor / Lode / App 对外 facts 与 Core 消费合同对齐。Core #16 已关闭，不再由本项阻塞 closeout。
- 当前状态：部分解决。Core–Harbor owner、failure attribution、敏感边界以及有界 compatibility/cutover/rollback 已由 [Core ADR 0010](0010-core-harbor-ownership-migration-v0.md)（#341）冻结；最终跨仓 wire field、API/version/hash pin 传输形式仍待规格化。
- 后续归属/下一步：Core #342、Harbor #352 及后续 Lode/App consumer Work Item 消费 #341/ADR 0010 并完成最终 wire schema 收敛；Core #16/#17/#18 只保留为历史来源，不作为活跃 closeout 目标。Core 继续“不复制其他仓 truth，只消费对外 refs / facts / intent”；字段/API 变化不得在 #341 docs-only PR 中猜测或落地。
