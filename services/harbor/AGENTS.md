...
- [#519](https://github.com/WebEnvoy/WebEnvoy/issues/519) B 的供应方原版路径只接受 owner 明确提供并核验的固定组合：Camoufox `0.5.6`、browser `152.0.4-beta.30`、Playwright `1.60.0`，由公开 API Driver 复用完整 launch/context options。其当前支持状态是 `limited`：popup 首请求无法在派发前建立可信 Page 归属时，Harbor 必须在 `fetch`/`continue`/外部请求前以 `page_relation_unavailable` 局部拒绝，不猜测或重放；已知 Page、普通任务页和同 Instance 观察/接管仍按公共合同运行。正式 installed、交还、环境连续性和真实 Agent 证据仍待 #519 完成门，不写成 `live_verified`。旧 Camoufox 私有 launch binding、patched/native artifact 继续 `unsupported`／已退役；Harbor 不启动、不 fallback，旧 #499/#504/#510 记录仅用于历史和 recovery 校验。
