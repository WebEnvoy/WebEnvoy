...
import { launchChromeOfficialProvider } from "./chrome-official-driver.js";
...
const CHROME_PATH = "/managed/chrome/Google Chrome";
...
const chromeBinding = {
  ...
  selected_provider: {
    ...
    selectable: true,
    install: { status: "installed", path: CHROME_PATH, launchability: "launchable", version: "152.0.7977.84" }
  }
} as LocalIdentityEnvironmentFacts["provider_binding"];
...
const launchCases = [
  ...
  },
  {
    ...
    launch: launchChromeOfficialProvider,
    pythonEnv: "HARBOR_PLAYWRIGHT_PYTHON",
    driverEnv: undefined,
    identity_environment: {
      provider_binding: chromeBinding,
      environment: {
        proxy: { state: "configured", proxy_ref: "proxy:shared-test", label: "managed" },
        geoip_mode: "system",
        language: null,
        timezone: null,
        viewport: "1280x900"
...
test("uses one shared launch factory for the public provider adapters", async () => {
  const root = await mkdtemp(join(tmpdir(), "harbor-shared-driver-factory-"));
  const helper = join(root, "fixture-driver.mjs");
  const pythonWrapper = join(root, "fixture-python");
...
for await (const line of rl) {
  ...
  try {
    ...
    for (const provider of launchCases) {
      if (provider.id === "chrome_official") {
        // These deliberately invalid Camoufox values prove the bound Chrome
        // case neither borrows source material nor falls back to Camoufox.
        process.env.HARBOR_CAMOUFOX_SOURCE = "untrusted";
        process.env.HARBOR_CAMOUFOX_SOURCE_SHA256 = "bad";
        process.env.HARBOR_CAMOUFOX_PYTHON = join(root, "missing-camoufox-python");
        process.env.HARBOR_CAMOUFOX_DRIVER = join(root, "missing-camoufox-driver");
        process.env.HARBOR_CHROME_PYTHON = join(root, "missing-legacy-chrome-python");
        process.env.HARBOR_CHROME_DRIVER = join(root, "missing-legacy-chrome-driver");
      } else {
      ...
      const driverPath = provider.id === "chrome_official" ? join(DRIVER_DIR, "chrome_official_driver.py") : helper;
      process.env.HARBOR_EXPECTED_DRIVER_PATH = driverPath;
      process.env[provider.pythonEnv] = provider.id === "chrome_official" ? pythonWrapper : process.execPath;
      ...
      const input: LocalProviderLaunchInput = {
        ...
        ...(provider.id === "chrome_official" ? { resolve_proxy: () => "http://proxy.example:8080/" } : {})
      ...
      assert.equal(result.facts.some(fact => fact.key.startsWith("provider.camoufox.")), provider.id === "camoufox");
      assert.equal(result.facts.some(fact => fact.key === "fixture.environment.proxy_server" && fact.value === (provider.id === "chrome_official" ? "http://proxy.example:8080/" : "")), true);
      assert.equal(result.facts.some(fact => fact.key === "fixture.environment.viewport" && fact.value === (provider.id === "chrome_official" ? '{"width":1280,"height":900}' : "null")), true);
      ...
      if (provider.id === "chrome_official") assert.equal("provider_id" in environment!.provider && environment.provider.provider_id, provider.id);
      else assert.equal("provider_id" in environment!.provider, false);
...
test("keeps Camoufox qualification fail-closed instead of selecting the shared Chrome adapter", async () => {
  const previous = { ...process.env };
  try {
...
test("packages the shared driver and both thin provider adapters", async () => {
  for (const name of ["playwright_shared_driver.py", "camoufox-upstream-driver.py", "chrome_official_driver.py"]) {
