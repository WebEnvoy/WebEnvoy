...
export const managedOperationCatalog = {
  ...
  catalog_ref: "harbor://managed-operations", catalog_version: "5",
  operations: [...["profile.list", "profile.read", "profile.create", "instance.start", "instance.stop", "instance.observe", "instance.diagnostics", "environment.read", "environment.update", "instance.navigate", "instance.read", "instance.handoff", "account.bind", "page.list", "page.open", "page.activate", "page.close", "page.navigate", "page.reload", "page.back", "page.forward"].map(operation_id => ({
    operation_id, category: operation_id === "environment.update" ? "prepare" : ["profile.create", "account.bind", "page.open", "page.activate", "page.close", "page.navigate", "page.reload", "page.back", "page.forward"].includes(operation_id) ? "prepare" : "read",
