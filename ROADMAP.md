...
Core 负责统一任务入口、执行准入、资源匹配、Run Record、Result Envelope、统一授权策略、失败归因和 API / CLI / MCP / SDK 公共任务路径。
...
Core 支持 validate-only、draft 和 preview 写前验证，能表达预期变更和所需授权，但不把预览当作真实提交结果。

可创建 milestone 的主题：

- action declaration 和统一授权策略 v0。
- 全局默认、任务覆盖、单次授权和 cancellation 语义。
- validate-only / preview Result Envelope。
- 写前验证失败和用户取消写入 Run Record。

### 组织阶段七投影：受控写入闭环

Core 支持首批低风险真实写入的明确授权、幂等、post-check、unknown outcome 和对账入口。

可创建 milestone 的主题：

- authorization-gated write execution。
...
- SDK / MCP / CLI 入口一致性验证。
- run history、business result、failure、authorization 和 recovery 查询稳定化；evidence 作为按需诊断记录。
- 多入口错误语义一致性。
