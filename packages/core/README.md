...
GH-293 records trusted evaluator outputs as immutable business decision summaries with append-only lifecycle events. Task decisions are referenced by Run Record and projected on the matching turn; environment operations use the same opaque decision-ref contract without copying Harbor private state.
