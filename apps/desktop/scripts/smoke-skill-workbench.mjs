...
const mediaFields = inputModule.projectInputFields(JSON.parse(mediaSchema), {
  packageRef: "lode://site-capability/xiaohongshu/publish-note-image-text-media@0.1.0",
  schemaId: "lode://schema/site-capability/xiaohongshu/publish-note-image-text-media/input@0.1.0",
...
for (const relativePath of (await readdir(inputSchemaRoot, { recursive: true })).filter((file) => file.endsWith("schemas/input.schema.json"))) {
  ...
  const projected = relativePath.includes("publish-note-image-text-media")
