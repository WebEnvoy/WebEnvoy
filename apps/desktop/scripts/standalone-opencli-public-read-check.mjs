...
try {
  ...
  for (const sample of samples) {
    ...
    if (attempted.run?.run_id && attempted.run.status !== 'succeeded') {
      ...
      const probeUrl = {
        devto: 'https://dev.to/api/articles/latest?per_page=2&page=1',
        arxiv: 'https://export.arxiv.org/api/query?search_query=cat%3Acs.AI&max_results=2&sortBy=submittedDate&sortOrder=descending',
      }[sample.name];
      if (probeUrl && attempted.failure?.code === 'managed_task_network_content_type_denied') {
        const probe = spawnSync('/usr/bin/curl', ['--silent', '--show-error', '--http1.1', '--max-time', '10',
          '--output', '/dev/null', '--write-out', '%{http_code} %{content_type}',
          ...(sample.name === 'devto' ? ['--header', 'accept: application/json'] : []), '--header', 'user-agent:',
