...
export function AgentAccessPanel({ endpoint }: { endpoint: string }) {
  ...
  const directFileScope = directSelectedFiles.length ? { upload_refs: directSelectedFiles.map(file => file.file_ref), allowed_mime_types: [...new Set(directSelectedFiles.map(file => file.mime_type))], max_file_bytes: Math.max(...directSelectedFiles.map(file => file.byte_length), 1) } : undefined;
  useEffect(() => {
  ...
  return <section className="settings-group" aria-label="Agent 接入管理">
    ...
    <div className="settings-group-content">
      ...
      <form onSubmit={event => { event.preventDefault(); if (!policyRef) return; if (v2PolicyEditing && !window.confirm("确认调整此 v2 Profile 权限上限？Core 会核对当前摘要，并要求 Harbor 保持 Profile 停止；已有 Grant 不会自动扩大。")) return; void mutate(v2PolicyEditing ? "/agent-access/v2/profile-policies" : "/agent-access/profile-policies", key => v2PolicyEditing ? createAgentOperationsV2PolicyInput(selectedPolicy!, policyScope, key) : createProfilePolicyInput(policyRef, policyScope, key)); }}>
      ...
      <form onSubmit={event => { event.preventDefault(); if (v2GrantCreation && selectedGrantPolicy) { if (!window.confirm(`确认直接签发 v2 Grant？Principal：${principalId}；Profile：${selectedGrantPolicy.profile_ref}；origin：${scope.origins?.filter(Boolean).join("、") || scope.origin}；操作：${scope.operations.join("、") || "无"}；文件 ref：${directSelectedFiles.map(file => file.file_ref).join("、") || "无"}；期限：${hours} 小时（提交时起算）；替换：否：直接新签发；来源：无；当前 policy 摘要 ${selectedGrantPolicy.policy_digest ?? "缺少，请刷新"} 会由 Core 再次核对。`)) return; void mutate("/agent-access/v2/grants", key => createAgentOperationsV2DirectGrantInput(principalId, selectedGrantPolicy, scope, key, hours, directFileScope)); return; } void mutate("/agent-access/grants", key => createAgentGrantInput(principalId, hours, key, scope, profileRef, templateProviderId)); }}>
        ...
        <label className="connection-field"><span>{v2GrantCreation ? "授权 v2 Profile" : "授权 Profile 或创建模板"}</span><select name="grant_profile" required={v2GrantCreation} value={profileRef} disabled={disabled} onChange={event => { setProfileRef(event.currentTarget.value); setDirectV2FileRefs([]); setScope(current => ({ ...current, controlled: false })); }}><option value="">{v2GrantCreation ? "请选择 v2 Profile" : "创建最多 2 个新 Profile"}</option>{state?.profile_policies.map(item => <option key={item.profile_ref} value={item.profile_ref}>{item.profile_ref} · {item.scope_semantics === "agent_operations_v2" ? "v2" : "legacy"}</option>)}</select></label>
        ...
        {v2GrantCreation && <fieldset disabled={disabled}><legend>选择现有 owner 文件材料</legend>{ownerFiles.filter(file => file.profile_ref === selectedGrantPolicy?.profile_ref && file.status === "available").length === 0 && <p>此 Profile 没有可批准的 owner 文件材料。</p>}{ownerFiles.filter(file => file.profile_ref === selectedGrantPolicy?.profile_ref && file.status === "available").map(file => <label key={file.file_ref} style={{ display: "block" }}><input type="checkbox" checked={directV2FileRefs.includes(file.file_ref)} onChange={event => { const checked = event.currentTarget.checked; setDirectV2FileRefs(current => checked ? [...new Set([...current, file.file_ref])] : current.filter(ref => ref !== file.file_ref)); }} />{file.display_name} · {file.mime_type} · {file.byte_length} bytes</label>)}</fieldset>}
      ...
      {v2GrantCandidates.length > 0 && <form onSubmit={event => {
        ...
        const fileScope = selectedFiles.length ? { upload_refs: selectedFiles.map(file => file.file_ref), allowed_mime_types: [...new Set(selectedFiles.map(file => file.mime_type))], max_file_bytes: Math.max(...selectedFiles.map(file => file.byte_length), 1) } : undefined;
