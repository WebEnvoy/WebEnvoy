...
Submit each intent through `webenvoy_operation` with a unique idempotency_key, grant_id and task_scope (operations, profile_refs, origins). The connection_id is inserted by the connector. Use only the owner-approved template_ref for profile.create. Profile creation never grants permission. Use profile.list/read and instance.start/observe/handoff/stop for management. Include origin explicitly for every instance operation, as well as in task_scope.origins. URLs in this slice must be http(s), match origin, and contain no query string, fragment, or embedded credentials; for example https://example.com/. After an input rejection, query the original key. Correct the parameters only when that query confirms no Run exists; a failed Run does not prove that nothing happened.
