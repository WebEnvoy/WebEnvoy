...
> **Current provider boundary (2026-09-12):** The background `page.open`, `active`/`opener_page_id`, and automatic safe-adjacent-Page rules below retain the old #510 wire and historical semantics for compatibility reading. They are not a current Camoufox capability recommendation. The private Camoufox browser/Driver launch binding is retired: it must not launch, create a new Profile, fallback, or auto-open/activate an adjacent Page; a Camoufox request is refused as unsupported without spawning the old binding. Provider-neutral Page/Document/navigation wire remains available where the selected Provider supports it. The current target boundary follows the [canonical v1 product architecture](https://github.com/WebEnvoy/.github/blob/main/docs/product-architecture-v1.md) and the [retirement disposition in the Camoufox contract](https://github.com/WebEnvoy/WebEnvoy/blob/main/docs/specs/camoufox-native-provider-contract-v1.md); any future Camoufox support requires a new qualification, contract, and acceptance path.
