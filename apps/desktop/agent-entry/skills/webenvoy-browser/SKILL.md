...
> **当前 Provider 边界（2026-09-12）：** 下文关于后台 `page.open`、`active`/`opener_page_id` 和自动安全邻页选择的说明，保留 #510 旧 wire 及其历史语义，仅用于兼容阅读；不得将其推荐为当前 Camoufox 能力。本轮 B 仅保留这些旧 wire 并禁用 Camoufox 私有浏览器／Driver 启动绑定，未实现原版 Camoufox 或原生任务页路径：不得启动、创建新 Profile、自动 fallback，或自动打开／激活邻页；Camoufox 请求必须以 `unsupported` 拒绝，且不得 spawn 旧 binding。选定 Provider 支持时，与 Provider 无关的 Page／Document／navigation wire 仍然有效。当前采用的规则和任务页协作边界以 [canonical v1 产品架构](https://github.com/WebEnvoy/.github/blob/main/docs/product-architecture-v1.md) 为准；本轮 B 不代表该 canonical 新任务页目标已实现。任何未来 Camoufox 支持都必须另行完成资格门槛、正式合同和验收路径。
