...
const tools = [
  ...
  { name: 'webenvoy_operation', description: 'Submit one authorized management/public-read/controlled-page operation. Keep exact runtime_session_ref. snapshot discovers bounded controls; click/input/press/scroll/wait require its page_ref and observation_ref. Never retries.', inputSchema: { type: 'object', properties: { idempotency_key: { type: 'string' }, grant_id: { type: 'string' }, operation: { type: 'string', enum: ['profile.create','profile.list','profile.read','instance.start','instance.observe','instance.navigate','instance.read','instance.snapshot','instance.click','instance.input','instance.press','instance.scroll','instance.wait','instance.handoff','instance.stop'] }, task_scope: { type: 'object', properties: { operations: { type: 'array', items: { type: 'string' } }, profile_refs: { type: 'array', items: { type: 'string' } }, origins: { type: 'array', items: { type: 'string' } } }, required: ['operations','profile_refs','origins'], additionalProperties: false }, template_ref: { type: 'string' }, profile_ref: { type: 'string' }, runtime_session_ref: { type: 'string' }, origin: { type: 'string' }, url: { type: 'string' }, page_ref: { type: 'string' }, observation_ref: { type: 'string' }, target_ref: { type: 'string' }, text: { type: 'string', maxLength: 512 }, key: { type: 'string', enum: ['Enter','Tab','ArrowDown','ArrowUp','ArrowLeft','ArrowRight','Home','End','Space','Backspace','Delete','Escape'] }, delta_y: { type: 'integer', minimum: -2000, maximum: 2000 }, wait_for: { type: 'string', enum: ['page_changed','text','enabled'] }, timeout_ms: { type: 'integer', minimum: 1, maximum: 10000 } }, required: ['idempotency_key','grant_id','operation','task_scope'], additionalProperties: false } },
