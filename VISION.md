...
- 按 Lode 输出契约校验能力结果，并返回公共 result envelope；
...
Core 不应把平台 raw payload 直接返回给上游系统，也不应让 API、CLI、MCP、SDK 和 App 各自解释站点字段。

Core 应消费 Lode 声明的 output schema，对能力执行结果进行校验、公共投影和 result envelope 封装。raw payload、截图、网络摘要和执行现场应通过 raw_payload_ref、evidence_ref 或 source_trace 引用，而不是直接进入公共结果。

这样，上游系统拿到的是稳定、可解释、可记录的结构化结果，而不是临时脚本返回的私有字段集合。
...
- Lode 负责站点知识、能力包、原子动作、任务封装、输出契约、模板、测试样例、版本和失效标记；
...
Core 不是通用 ETL、爬虫数据清洗平台或业务数据仓库；它只负责网站能力执行结果的公共投影、校验、封装和记录。
