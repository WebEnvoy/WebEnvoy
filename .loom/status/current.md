...
- Current Stop: Corrective implementation is open as PR #257 at head `7822bd3fc9f87ef5ae2ce9c81a31d2b7d2a89217`: `verifyEvidenceRefs` now requires each Harbor evidence lookup response to match the requested `evidence_ref` and report `access_state: available`, with self-check coverage for invalid scene URLs plus malformed and mismatched evidence lookup responses failing closed without copied `evidence_refs`.
