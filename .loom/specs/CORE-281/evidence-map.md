...
| EV-003 | behavior_evidence | .loom/specs/CORE-281/spec.md packages/core/src/lode-admission.ts packages/core/src/runtime-task-chain.ts | Shared policy validation, pre-Harbor rejection, and XHS preservation | CORE-281 acceptance | present | Core admission behavior only | Refresh after behavior or boundary changes. |
| EV-004 | test_evidence | packages/core/src/self-check.ts packages/api-server/src/runtime-task-submit-self-check.ts .loom/progress/CORE-281.md | Direct Core, five entrypoints, three BOSS operations, zero Harbor calls, drift rejection, XHS success, and full checks | CORE-281 validation | present | synthetic tests and repository validation only; no production page evidence | Refresh after code, tests, or validation changes. |
