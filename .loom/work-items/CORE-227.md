...
- Product writes are limited to `packages/core/src/runtime-task-chain.ts`, `packages/api-server/src/server.ts`, `packages/api-server/src/runtime-task-submit-self-check.ts`, and `packages/api-server/src/runtime-process-self-check.ts`.
